...
public class BannedwordsProc implements BannedwordsProcInter {
    ...
    private BannedwordsDAOInter bannedwordsDAO;

    public BannedwordsProc() {
        System.out.println("-> BannedwordsProc created.");
    }
    
    @Override
    public int create(BannedwordsVO bannedwordsVO) {
        int cnt = this.bannedwordsDAO.create(bannedwordsVO);
        return cnt;
    }
    
    @Override
    public ArrayList<BannedwordsVO> list_all() {
        ArrayList<BannedwordsVO> list = this.bannedwordsDAO.list_all();
        return list;
    }
    
    @Override
    public ArrayList<BannedwordsVO> list_by_wordno(int wordno) {
      ArrayList<BannedwordsVO> list = this.bannedwordsDAO.list_by_wordno(wordno);
      return list;
    }
    ...
    public ArrayList<BannedwordsVO> list_by_wordno_search_paging(HashMap<String, Object> map) {
      ...
      // 데이터베이스 쿼리 실행
      ArrayList<BannedwordsVO> list = this.bannedwordsDAO.list_by_wordno_search_paging(map);
      return list;
    }
   
    @Override
    public String pagingBox(int now_page, String word, String reason, int search_count,
      ...
      if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성
        ...
        str.append("<span class='span_box_1'><A href='" + reason + "?word=" + word + "&now_page=" + _now_page
            + "'>이전</A></span>");
      }
      ...
      for (int i = start_page; i <= end_page; i++) {
        ...
        if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
          ...
          str.append("<span class='span_box_1'><A href='" + reason + "?word=" + word + "&now_page=" + i + "'>" + i
              + "</A></span>");
        }
      }
    ...
    @Override
    public int count_by_wordno_search(HashMap<String, Object> map) {
      int cnt = this.bannedwordsDAO.count_by_wordno_search(map);
      return cnt;
    }
    
    @Override
    public ArrayList<BannedwordsVO> list_by_wordno_search(HashMap<String, Object> hashMap) {
      ArrayList<BannedwordsVO> list = this.bannedwordsDAO.list_by_wordno_search(hashMap);
      return list;
    }

    @Override
    public int update_text(BannedwordsVO bannedwordsVO) {
        int cnt = this.bannedwordsDAO.update_text(bannedwordsVO);
        return cnt;
    }
    
    @Override
    public int delete(int wordno) {
        int cnt = this.bannedwordsDAO.delete(wordno);
        return cnt;
    }
