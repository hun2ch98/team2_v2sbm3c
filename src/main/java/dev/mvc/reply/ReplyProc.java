...
public class ReplyProc implements ReplyProcInter {
  ...
  public int create(ReplyVO replyVO) {
    ...
    replyVO.setPasswd(passwd_encoded);
    // -------------------------------------------------------------------
  ...
  public ArrayList<ReplyVO> list_all() {
    ArrayList<ReplyVO> list = this.replyDAO.list_all();
    return list;
  }

  @Override
  public ArrayList<ReplyVO> list_by_replyno(int replyno) {
    ArrayList<ReplyVO> list = this.replyDAO.list_by_replyno(replyno);
    return list;
  }
  ...
  public ArrayList<ReplyVO> list_by_boardno_search_paging(HashMap<String, Object> map) {
     ...
     */
    int begin_of_page = ((int)map.get("now_page") - 1) * Reply.RECORD_PER_PAGE;
    ...
    ArrayList<ReplyVO> list = this.replyDAO.list_by_replyno_search_paging(map);
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
  ...
  public int password_check(HashMap<String, Object> map) {
    ...
    int cnt = this.replyDAO.password_check(map);
    return cnt;
  }
  ...
  public int count_by_boardno(int boardno) {
    int cnt = this.replyDAO.count_by_boardno(boardno);
    return cnt;
  }

  @Override
  public int delete_by_boardno(int boardno) {
    int cnt = this.replyDAO.delete_by_boardno(boardno);
    return cnt;
  }

  @Override
  public int count_by_memberno(int memberno) {
    int cnt = this.replyDAO.count_by_memberno(memberno);
    return cnt;
  }

  @Override
  public int delete_by_memberno(int memberno) {
    int cnt = this.replyDAO.delete_by_memberno(memberno);
    return cnt;
  }

  @Override
  public int increaseReplycnt(int replyno) {
    int count = replyDAO.increaseReplycnt(replyno);
    return count;
  }

  @Override
  public int decreaseReplycnt(int replyno) {
    int count = replyDAO.decreaseReplycnt(replyno);
    return count;
  }
  
  @Override
  public ArrayList<ReplyVO> listByBoardNo(int boardno) {
      return replyDAO.listByBoardNo(boardno);  // DAO에서 게시물 번호로 댓글 리스트를 가져옴
  }

  @Override
  public int deleteByBoardNo(int boardno) {
      return replyDAO.deleteByBoardNo(boardno);  // 해당 게시물 번호로 모든 댓글 삭제
  }
