DROP TABLE schedule;

-- Oracle 12C+
--CREATE TABLE schedule (
--  scheduleno  NUMBER GENERATED BY DEFAULT AS IDENTITY(START WITH 1 INCREMENT BY 1) PRIMARY KEY, -- AUTO_INCREMENT 대체
--  sdate   VARCHAR2(10)  NOT NULL, -- 출력할 날짜 2013-10-20
--  label       VARCHAR2(50)  NOT NULL, -- 달력에 출력될 레이블
--  title       VARCHAR2(100) NOT NULL, -- 제목(*)
--  content     CLOB          NOT NULL, -- 글 내용
--  cnt         NUMBER        DEFAULT 0, -- 조회수
--  seqno       NUMBER(5)     DEFAULT 1 NOT NULL, -- 일정 출력 순서
--  regdate     DATE          NOT NULL -- 등록 날짜
--);

-- 수업
CREATE TABLE schedule (
  scheduleno  NUMBER(10) NOT NULL PRIMARY KEY, -- AUTO_INCREMENT 대체
  sdate   VARCHAR2(10)  NOT NULL, -- 출력할 날짜 2013-10-20
  label       VARCHAR2(50)  NOT NULL, -- 달력에 출력될 레이블
  title       VARCHAR2(100) NOT NULL, -- 제목(*)
  content     CLOB          NOT NULL, -- 글 내용
  seqno       NUMBER(5)     DEFAULT 1 NOT NULL, -- 일정 출력 순서
  regdate     DATE          NOT NULL, -- 등록 날짜
  memberno    NUMBER(10)     NOT NULL , -- FK
  FOREIGN KEY (memberno) REFERENCES member (memberno) -- 일정을 등록한 관리자 
);

DROP SEQUENCE schedule_seq;

CREATE SEQUENCE schedule_seq
START WITH 1         -- 시작 번호
INCREMENT BY 1       -- 증가값
MAXVALUE 9999999999  -- 최대값: 9999999999 --> NUMBER(10) 대응
CACHE 2              -- 2번은 메모리에서만 계산
NOCYCLE;             -- 다시 1부터 생성되는 것을 방지


-- 데이터 삽입
INSERT INTO schedule(scheduleno, sdate, label, title, content, seqno, regdate, memberno)
VALUES (schedule_seq.nextval, '2024-12-24', '크리스마스 이브', '메리 크리스마스', '즐거운 크리스마스 되세요.', 1, sysdate, 1);

INSERT INTO schedule(scheduleno, sdate, label, title, content, seqno, regdate, memberno)
VALUES (schedule_seq.nextval, '2024-12-25', '휴강 안내', '메리 크리스마스', '휴강입니다.', 1, sysdate, 1);

INSERT INTO schedule(scheduleno, sdate, label, title, content, seqno, regdate, memberno)
VALUES (schedule_seq.nextval, '2024-12-25', '학원 출입 안내', '학원 출입 안내', '크리스마스 기간에 학원 입실 안됩니다.', 2, sysdate, 1);

INSERT INTO schedule(scheduleno, sdate, label, title, content, seqno, regdate, memberno)
VALUES (schedule_seq.nextval, '2025-01-01', '새해 첫날 학원 출입 안내', '새해 첫날 학원 출입 안내', '새해 첫날에 학원 입실 안됩니다.', 2, sysdate, 1);

-- 전체 목록
SELECT scheduleno, sdate, label, title, content, seqno, regdate, memberno
FROM schedule
ORDER BY scheduleno DESC;

-- 특정 달의 목록
SELECT scheduleno, sdate, label, seqno
FROM schedule
WHERE TO_CHAR(TO_DATE(sdate, 'YYYY-MM-DD'), 'YYYY-MM') = '2024-12'
ORDER BY sdate ASC, seqno ASC;

CALENDARNO LABELDATE  LABEL                               SEQNO
---------- ---------- ------------------------------ ----------
         1 2024-12-24 크리스마스 이브                         1
         2 2024-12-25 휴강 안내                               1
         3 2024-12-25 학원 출입 안내                          2

-- 특정 날짜의 목록
SELECT scheduleno, sdate, label, seqno
FROM schedule
WHERE sdate = '2024-12-24';

CALENDARNO LABELDATE  LABEL                                                   SEQNO
---------- ---------- -------------------------------------------------- ----------
         1 2024-12-24 크리스마스 이브                                             1

-- 조회수 증가
UPDATE schedule
SET cnt = cnt + 1
WHERE scheduleno = 1;

-- 조회
SELECT scheduleno, sdate, label, title, content, regdate, seqno
FROM schedule
WHERE scheduleno = 1;

-- 변경
UPDATE schedule
SET sdate = '', label = '', title = '', content = '', seqno = 1
WHERE scheduleno = 1;

-- 삭제
DELETE FROM schedule
WHERE scheduleno = 1;



