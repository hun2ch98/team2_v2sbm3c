...
ORDER BY goodno DESC;
 ...
 -- 2개 조인
SELECT g.goodno, g.rdate, g.surveyno, s.topic, g.memberno
FROM survey s, surveygood g
WHERE s.surveyno = g.surveyno
ORDER BY goodno DESC;

 -- 3개 조인
SELECT g.goodno, g.rdate, g.surveyno, s.topic, g.memberno, m.id, m.email
